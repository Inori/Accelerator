...
def makestr(lines):
    ...
    for index,line in enumerate(lines):
        part = re.match('○([0-9A-Fa-f]+)○[0-9]+●', line)
        if part:
            offset = int(part.group(1), base=16)
            oldstr = lines[index+1][:-1]
            newstr = lines[index+1][:-1]

            offset_list.append(offset)
            oldstr_list.append(fixjpstr(oldstr))
            newstr_list.append(fixcnstr(newstr))

    return offset_list, oldstr_list, newstr_list
'''
def makestr(lines):
    oldstr_list = []
    newstr_list = []
...
def WriteString(src, offset_list,  oldstr_list, newstr_list):
    index = []
    for offset, oldstr, newstr in zip(offset_list, oldstr_list, newstr_list):

        oldoff = src.tell()
        bstring = oldstr.encode('gbk', errors='ignore')
        oldlen = len(bstring)
        src.write(bstring)

        newoff = src.tell()
        bstring = newstr.encode('gbk', errors='ignore')
        newlen = len(bstring)
        src.write(bstring)

        index.append([offset, oldoff, oldlen, newoff, newlen])
...
def Main():
    ...
    offset_list, oldstr_list, newstr_list = makestr(lines)
    ...
    index_list, filesize = WriteString(dst,offset_list, oldstr_list, newstr_list)
